  ...
  const validateField = (name: string, value: string): string | undefined => {
    switch (name) {
      ...
      case 'phone':
        ...
        break;
      case 'serviceType':
        if (!validateRequired(value)) return 'Please select a service type';
        break;
      case 'message':
        if (!validateRequired(value)) return 'Message is required';
        if (!validateMinLength(value, 20)) return 'Message must be at least 20 characters';
        break;
    }
    return undefined;
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              ...
              <div>
                ...
                {errors.serviceType && touched.serviceType && (
                  <p className={errorClasses}>{errors.serviceType}</p>
                )}
              </div>
